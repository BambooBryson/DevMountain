...
// then loop through the array (that will be passed in as a parameter) to see if that random number is in the array. If it is, return true, if it's not, return false
  ...
  //Code Here
var myRandom = getRandomArbitrary();

  //Code Here

function finder(givenNumber){
    for (var i = 0 ; i < givenNumber.length ; i++){

        if(givenNumber[i] === getRandomArbitrary()){
            return true;
        }
    }
    return false;
...
//Write a function called reverse that takes a given str as it's only argument and returns that string after it's been reversed
 ...
 function reverse(str){
    ...
    var returnedString = String(reverseArray);
